(function(factory) {
	...
	function createCanvas(context, constructor, selector, options) {
		if (
			canvasSupport &&
			(context.container = isElem(selector)
				? selector
	...
	var Particleground = {
		...
		commonConfig: { opacity: 1, color: [], resize: true },
		util: util,
		inherit: {
...
+(function(Particleground) {
	...
	Lowpoly.defaultConfig = { color: '#fff', maxSpeed: 0.6, minSpeed: 0 };
	...
	var fn = (Particle.prototype = {
		...
		init: function() {
			if (this.set.num > 0) {
				if (this.set.range > 0) {
					if (
						!util.isElem(this.set.eventElem) &&
			...
			dots.forEach(function(v, i) {
				...
				while (++i < length) {
					...
					if (
						abs(vx - sx) <= dis &&
						abs(vy - sy) <= dis &&
						((abs(vx - posX) <= posR && abs(vy - posY) <= posR) ||
							(abs(sx - posX) <= posR && abs(sy - posY) <= posR))
					) {
						cxt.save();
						cxt.beginPath();
						cxt.moveTo(vx, vy);
		...
		getElemOffset: function() {
			return (this.elemOffset = this.elemOffset
				? util.offset(this.set.eventElem)
				: null);
		},
		event: function() {
